...
def stabilised_value_learning(learning_rate, matches, candidates, iterations_for_learning=100, iterations_to_stabilise=100, cache=True):
    """
...
def extract_human_scores_system_level(data, convert_data_to_doc_text_system_rating_format, cache=True):
    ...
    # print("extract_human_scores_system_level")
...
def get_function_add_metric_results(metric_functions, eval_as_nn=["res"]):
...
#    start = time.time()
    ...
    data_set = data_set.map(get_function_add_metric_results(metric_functions), batched=True, num_proc=1,
                            batch_size=12000, desc="calculating the metric scores for each summary-doc pair")

    # for k, v in metric_functions:
    #     data_set[k]

#    print(f"metrics {metric_functions.keys()} took {time.time() - start}")
